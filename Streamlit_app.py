...
<body class="min-h-screen">
        ...
        <div class="flex justify-center">
            <div class="se-card rounded-xl p-8 glow-effect w-full max-w-3xl">
                ...
                <form id="analysisForm" class="space-y-8">
                    ...
                    <!-- Submit Button -->
                    <button type="button" id="runAnalysis" class="w-full py-4 px-8 se-gradient rounded-lg text-white text-xl font-medium hover:scale-105 focus:outline-none" disabled>
                        Run Analysis
                    </button>
                </form>
            </div>
    ...
    <script>
        // JavaScript interactivity for file input and submit button
        document.getElementById('analysisType').addEventListener('change', function() {
            const fileInput = document.getElementById('fileInput');
            const submitButton = document.getElementById('runAnalysis');
            if (this.value) {
        ...
        document.getElementById('runAnalysis').addEventListener('click', function() {
            ...
            if (files.length > 0 && analysisType) {
                // Trigger Streamlit file upload
                const fileUploadEvent = new CustomEvent('fileUpload', { detail: { files, analysisType } });
                window.parent.document.dispatchEvent(fileUploadEvent);
            }
        });
    </script>
</body>
