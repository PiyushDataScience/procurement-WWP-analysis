...
def run(open_po_bef, wb):
    ...
    new_column_order = [
        ...
        'VENDOR_NAME', 'VENDOR_DUNS', 'VENDOR_NUM', 'STARS Category Code',
        'PO_NUM', 'RELEASE_NUM', 'LINE_NUM', 'SHIPMENT_NUM',
        'AUTHORIZATION_STATUS', 'PO_SHIPMENT_CREATION_DATE',
        'QTY_ELIGIBLE_TO_SHIP', 'Unit_Price_WB', 'CURRENCY_CODE_WB',
    ...
    # Function to map Vendor to IG/OG based on Vendor Name
    def map_vendor_to_ig_og(vendor_name):
        return 'IG' if 'SCHNEIDER' in vendor_name or 'WUXI' in vendor_name else 'OG'

    # Apply the function to the 'VENDOR_NAME' column
